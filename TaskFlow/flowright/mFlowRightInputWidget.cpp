#include "mFlowRightInputWidget.h"
...
MFlowRightInputWidget::MFlowRightInputWidget(QWidget *parent,MFlowStepData* data) :
    QWidget(parent)
{
    m_data = data;
    m_params = static_cast<MFlowStepParamsInput*>(data->stepparams);
...
//    initRetryUI();
    ...
    connect(rightSave,&MRightSave::notifySave,this,&MFlowRightInputWidget::onSave);
    boxLayout->addWidget(rightSave);
    boxLayout->addStretch(10);

}
void MFlowRightInputWidget::onSave(bool clicked){
    Q_UNUSED(clicked);
    ...
    m_params->inputText = inputTextLine->text();
    m_params->isLoop = loopCheck->checkState()==Qt::CheckState::Checked?true:false;
}
void MFlowRightInputWidget::initInfoUI(){
...
void MFlowRightInputWidget::initSenierOptionUI(){
    ...
    ++optionBox->rowStart;
      ...
      ++optionBox->rowStart;
        QLabel *inputTextLabel = new QLabel(optionBox->gWidget);
        inputTextLabel->setText(tr("要输入的文本"));
        inputTextLine = new QLineEdit(optionBox->gWidget);
        inputTextLine->setStyleSheet(m_stylesheet_QLineEdit);
        inputTextLine->setText(this->m_params->inputText);
        optionBox->gLayout->addWidget(inputTextLabel,optionBox->rowStart,0);
        optionBox->gLayout->addWidget(inputTextLine,optionBox->rowStart,1);
    ++optionBox->rowStart;
...
MFlowRightInputWidget::~MFlowRightInputWidget()
{
